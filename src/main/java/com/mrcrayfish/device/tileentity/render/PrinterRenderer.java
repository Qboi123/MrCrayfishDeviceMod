 ...
 * @author MrCrayfish
 */
public class PrinterRenderer extends TileEntitySpecialRenderer<PrinterTileEntity>
{
    private static final ModelPaper MODEL_PAPER = new ModelPaper();

    @Override
    public void render(PrinterTileEntity te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
    {
        ...
        GlStateManager.pushMatrix();
        {
            ...
            if(te.hasPaper())
            {
                GlStateManager.pushMatrix();
                {
                    ...
                    GlStateManager.rotate(state.getValue(PrinterBlock.FACING).getHorizontalIndex() * -90F, 0, 1, 0);
